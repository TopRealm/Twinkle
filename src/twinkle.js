...
$.ajax({
	...
	.done((optionsText) => {
		...
		const optionsText_nowiki = optionsText.replace(
			/^\s+|^\n$|\/(\*|\/)\s+?<\/?nowiki>\s+?(\*\/)?\n?/g,
			''
		);
...
Twinkle.load = () => {
	...
	let activeSpecialPageList = [
		'Block',
		'Contributions',
		'AbuseLog',
		'Recentchanges',
		'Recentchangeslinked'
	]; // wgRelevantUserName defined for non-sysops on Special:Block
	...
	Twinkle.disabledModules = Twinkle.getPref('disabledModules').concat(
		Twinkle.getPref('disabledSysopModules')
	);
