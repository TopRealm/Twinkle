					...
					metadata.push(
						`全保护${
							editProt.expiry === 'infinity'
								? '（无限期）'
								: `（${new Morebits.date(editProt.expiry).calendar(
									'utc'
								)} (UTC)过期）`
						}`
					);
...
Twinkle.batchdelete.callback.change_common_reason = (e) => {
	if (e.target.form.reason.value !== '') {
		e.target.form.reason.value = Morebits.string.appendPunctuation(
...
Twinkle.batchdelete.callback.toggleSubpages = (e) => {
	...
	if (e.target.checked) {
		...
		if (subpagesLoaded) {
			...
			Morebits.quickForm
				.getElements(newPageList, 'pages')
				.forEach(Twinkle.generateArrowLinks);
			Morebits.quickForm
				.getElements(newPageList, 'pages.subpages')
				.forEach(Twinkle.generateArrowLinks);
			return;
		}
							...
							if (editProt) {
								metadata.push(
									`全保护（${
										editProt.expiry === 'infinity'
											? '永久'
											: `至${new Morebits.date(editProt.expiry).calendar(
												'utc'
											)} (UTC)过期`
									}`
								);
...
Twinkle.batchdelete.callback.evaluate = (event) => {
	...
	if (
		numProtected > 0 &&
			!confirm(`您将删除${mw.language.convertNumber(numProtected)}个全保护页面，您确定吗？`)
	) {
		return;
	}
	...
	const pageDeleter = new Morebits.batchOperation(
		input.delete_page ? '正在删除页面' : '正在启动要求的任务'
	);
	...
	pageDeleter.run(
		(pageName) => {
			...
			if (input.delete_page) {
				...
				qiuwen_page.deletePage(
					Twinkle.batchdelete.callbacks.doExtras,
					pageDeleter.workerFailure
				);
			} else {
...
Twinkle.batchdelete.callbacks = {
	...
	deleteRedirectsMain: (apiobj) => {
		...
		redirectDeleter.run((pageName) => {
			...
			qiuwen_page.setEditSummary(
				`[[QW:G9|G9]]：孤立页面（已删页面“${apiobj.params.page}”的讨论页）`
			);
			qiuwen_page.setChangeTags(Twinkle.changeTags);
			qiuwen_page.deletePage(
				redirectDeleter.workerSuccess,
				redirectDeleter.workerFailure
			);
		});
	},
	deleteTalk: (apiobj) => {
	...
	unlinkImageInstances: (pageobj) => {
		...
		if (text === old_text) {
			pageobj
				.getStatusElement()
				.error(`在 ${pageobj.getPageName()} 上取消 ${image} 的文件使用失败`);
