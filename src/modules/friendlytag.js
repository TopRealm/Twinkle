...
Twinkle.tag.callback = () => {
	...
	switch ( Twinkle.tag.mode ) {
		case "article":
			...
			var obj_values = Object.values || ( ( obj ) => Object.keys( obj ).map( ( key ) => {
				return obj[ key ];
			} ) );
...
Twinkle.tag.redirectList = {
	...
	"Navigation aids": {
		...
		Disambiguation: [ {
			...
			description: 'redirect from an ambiguous page name to a page that disambiguates it. This template should never appear on a page that has "(disambiguation)" in its title, use R to disambiguation page instead'
		}, {
...
Twinkle.tag.callbacks = {
		...
		var postRemoval = () => {
			...
			pageobj.save( () => {
				...
				if ( params.mergeTagOther ) {
					...
					var otherpage = new Morebits.wiki.page( params.mergeTarget, `Tagging other page (${params.mergeTarget})` );
	...
	redirect: ( pageobj ) => {
		var params = pageobj.getCallbackParameters(), pageText = pageobj.getPageText(), tagRe, tagText = "", summaryText = "Added", tags = [], i;
		...
		if ( pageText.match( /{{(?:redr|this is a redirect|r(?:edirect)?(?:.?cat.*)?[ _]?sh)/i ) ) {
			...
			var pageTags = pageText.match( /\s*{{R(?:edirect)? .*?}}/gim );
			var oldPageTags = "";
			if ( pageTags ) {
			...
			$.each( params.tags, ( _k, tag ) => {
				...
				switch ( tag ) {
					...
					case "Orphaned non-free revisions":
						...
						text = text.replace( /\{\{\s*(Template\s*:\s*)?(Non-free reduce|FairUseReduce|Fairusereduce|Fair Use Reduce|Fair use reduce|Reduce size|Reduce|Fair-use reduce|Image-toobig|Comic-ovrsize-img|Non-free-reduce|Nfr|Smaller image|Nonfree reduce)\s*(\|(?:\{\{[^{}]*\}\}|[^{}])*)?\}\}\s*/gi, "" );
...
Twinkle.tag.callback.evaluate = ( e ) => {
	...
	switch ( Twinkle.tag.mode ) {
		...
		case "file":
			...
			if ( params.tags.indexOf( "Do not move to Commons" ) !== -1 && params.DoNotMoveToCommons_expiry && ( !/^2\d{3}$/.test( params.DoNotMoveToCommons_expiry ) || parseInt( params.DoNotMoveToCommons_expiry, 10 ) <= new Date().getFullYear() ) ) {
