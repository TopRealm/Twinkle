...
$(function TwinkleBlock() {
			...
			list: [
				{
					...
					value: 'block',
					tooltip: wgULS(
						'用选择的选项全站封禁相关用户，如果未勾选部分封禁则为全站封禁。',
						'用選擇的選項全站封鎖相關使用者，如果未勾選部分封鎖則為全站封鎖。'
					),
				...
				},
				{
					...
					tooltip: wgULS(
						...
						'如果執行封鎖的管理員忘記發出封鎖模板，或你封鎖了使用者而沒有給其發出模板，則你可以用此來發出合適的模板。勾選部分封鎖以使用部分封鎖模板。'
					),
				...
				},
				{
					...
					tooltip: partialBox
						? wgULS(
								...
								'如果使用部分封鎖，不應選擇此項，除非您也想要禁止編輯使用者討論頁。'
						  )
						: '',
				},
			];
	...
	Twinkle.block.callback.filtered_block_groups = (group, show_template) =>
		$.map(group, (blockGroup) => {
			...
			const list = $.map(blockGroup.list, (blockPreset) => {
				...
				const registrationRestrict = blockSettings.forRegisteredOnly
	...
	Twinkle.block.callback.main = (pageobj) => {
		...
		let summary = params.usertalk_summary; // zhwiki
