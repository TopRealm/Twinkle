	...
	Twinkle.block.fetchUserInfo(() => {
		if (Twinkle.block.isRegistered) {
			...
			Morebits.quickForm.setElementVisibility(
				$form.find('[name=actiontype][value=tag]').parent(),
				true
			);
...
Twinkle.block.fetchUserInfo = (fn) => {
	...
	api.get(query).then(
		(data) => {
			...
			Twinkle.block.manualUnblock =
					Twinkle.block.hasBlockLog && data.query.logevents[0].action === 'unblock';
...
Twinkle.block.callback.change_action = (e) => {
		...
		Morebits.status.warn(
			Twinkle.block.currentBlockInfo
				...
				: [
					...
					$(
						`<b>${new Morebits.date(
							Twinkle.block.recentBlockLog.timestamp
						).calendar('utc')}</b>`
					)[0],
					`被${Twinkle.block.recentBlockLog.user}封禁`,
					$(
						`<b>${Morebits.string.formatTime(
							Twinkle.block.recentBlockLog.params.duration
						)}</b>`
					)[0],
	...
	if (blockBox && Twinkle.block.currentBlockInfo) {
		...
		let statusStr =
				relevantUserName +
				(Twinkle.block.currentBlockInfo.partial === '' ? '已被部分封禁' : '已被全站封禁');
		if (Twinkle.block.currentBlockInfo.expiry === 'infinity') {
			statusStr += '（无限期）';
		} else if (new Morebits.date(Twinkle.block.currentBlockInfo.expiry).isValid()) {
			statusStr += `（终止于${new Morebits.date(
...
Twinkle.block.transformBlockPresets = () => {
	...
	if (!Twinkle.block.blockGroupsUpdated) {
		$.each(
			Twinkle.block.blockGroups.concat(Twinkle.block.blockGroupsPartial),
			(_, blockGroup) => {
				...
				$.each(blockGroup.list, (_, blockPreset) => {
					...
					if (
						blockGroup.custom &&
							Twinkle.block.blockPresetsInfo[blockPreset.value] === undefined
					) {
						Twinkle.block.blockPresetsInfo[newPreset].reasonParam = true;
						Twinkle.block.blockPresetsInfo[blockPreset.value] =
								Twinkle.block.blockPresetsInfo[newPreset];
					}
					if (
						blockGroup.custom &&
							Twinkle.block.blockPresetsInfo[blockPreset.value].expiry === undefined
					) {
	...
	$.map(group, (blockGroup) => {
		...
		const list = $.map(blockGroup.list, (blockPreset) => {
			...
			if (!(blockSettings.templateName && show_template) && registrationRestrict) {
				const templateName =
						blockSettings.templateName || blockSettings.template || blockPreset.value;
				return {
					label:
...
Twinkle.block.callback.evaluate = (e) => {
	...
	if (toTag) {
		...
		if (
			checkIncompatible(
				['Blocked sockpuppet', 'Sockpuppeteer'],
				'请从主账户和分身账户中选择一个。'
			)
		) {
			return;
		...
		if (blockoptions.partial) {
			...
			if (!blockoptions.namespacerestrictions && !blockoptions.pagerestrictions) {
				if (!blockoptions.noemail && !blockoptions.nocreate) {
					...
					!confirm('您将要进行封禁，但没有阻止任何页面或命名空间的编辑，确定要继续？')
				) {
					return;
				}
			}
...
Twinkle.block.callback.closeRequest = (vipPage) => {
	...
	const comment = `{{Blocked|${
		Morebits.string.isInfinity(params.expiry) ? 'indef' : expiryText
	}}}。`;
...
Twinkle.block.callback.main = (pageobj) => {
	...
	params.expiry =
			typeof params.template_expiry !== 'undefined' ? params.template_expiry : params.expiry;
