...
$(function TwinkleProtect() {
			...
			list: [
				{
					label: '请求保护页面',
					value: 'request',
					tooltip: `若您想在管理员通告版请求保护此页${Morebits.userIsSysop ? '而不是自行完成。' : '。'}`,
	...
	Twinkle.protect.callback.changePreset = (event) => {
		...
		if (actiontype === 'protect') {
			...
			if (mw.config.get('wgArticleId') && mw.config.get('wgPageContentModel') !== 'Scribunto') {
				form.tagtype.value =
					form.category.value === 'unprotect' ? 'none' : item.template || form.category.value;
			...
			case 'request': {
				...
				switch (input.category) {
					...
					case 'unprotect': {
						...
						if (
							admins.length > 0 &&
							!confirm(`是否已与管理员 (${Morebits.array.uniq(admins).join(', ')})沟通？`)
						) {
							return false;
						}
	...
	Twinkle.protect.callbacks = {
		...
		closeRequest: (rppPage) => {
			...
			text =
				params.type === 'unprotect' ? sections[0] + requestList.join('') : requestList.join('') + sections[1];
			let summary = '';
