...
Twinkle.arv = () => {
	...
	const windowTitle = mw.util.isIPAddress(username)
		? '报告IP地址给管理员'
		: '报告用户给管理员';
	...
	switch (value) {
		...
		default: {
			...
			work_area.append({
				...
				disabled:
						...
						mw.util.getParamValue('vanarticlerevid')
			});
			work_area.append({
				type: 'checkbox',
				name: 'arvtype',
			...
			work_area.append({
				...
				list: [
					...
					},
					{
						...
						value: 'promotional',
						tooltip:
								'不得注册宣传性用户名，不得以组织名义注册账号。不得以用户名宣扬特定政治及宗教观点。'
					},
					{
						label: '违背公序良俗的用户名',
						value: 'illegal',
						tooltip:
								'包括但不限于：违反法律规定的用户名，可能引发严重争议的用户名，仿冒他人的用户名，侮辱性或破坏性用户名等'
					},
					{
		...
		case 'puppet': {
			...
			work_area.append({
				...
				name: 'evidence',
				tooltip:
						'输入能够用来体现这些用户可能滥用多重账户的证据，这通常包括互助客栈发言、页面历史或其他有关的信息。请避免在此处提供非与傀儡或滥用多重账户相关的其他讨论。'
			});
			work_area.append({
				type: 'checkbox',
				list: [
	...
	switch (form.category.value) {
		...
		default: {
			...
			aivPage.load(() => {
				...
				if (
					new RegExp(
						`\\{\\{\\s*(?:(?:[Ii][Pp])?[Vv]andal|[Uu]serlinks)\\s*\\|\\s*(?:1=)?\\s*${Morebits.string.escapeRegExp(
							uid
						)}\\s*\\}\\}`
					).test(text)
				) {
		...
		case 'username': {
			...
			Morebits.wiki.actionCompleted.redirect =
					'Qiuwen:Usernames for administrator attention';
		...
		case 'puppet': {
			...
			sockParameters.sockpuppets = puppetReport
				? [uid]
				: Morebits.array.uniq(
					$.map($('input:text[name=sockpuppet]', form), (o) => $(o).val() || null)
				);
			Morebits.simpleWindow.setButtonsEnabled(false);
			Morebits.status.init(form);
		...
		case 'an3': {
			const diffs = $.map($('input:checkbox[name=s_diffs]:checked', form), (o) =>
				$(o).data('revinfo')
			);
			if (
				diffs.length < 3 &&
			...
			const warnings = $.map($('input:checkbox[name=s_warnings]:checked', form), (o) =>
				$(o).data('revinfo')
			);
			if (
				!warnings.length &&
			...
			const resolves = $.map($('input:checkbox[name=s_resolves]:checked', form), (o) =>
				$(o).data('revinfo')
			);
...
Twinkle.arv.processSock = (params) => {
	...
	spiPage.setEditSummary(
		`加入对[[Special:Contributions/${params.uid}|${params.uid}]]的新提报`
	);
...
Twinkle.arv.processAN3 = (params) => {
	...
	new mw.Api()
		...
		.done((data) => {
			...
			let origtext = '';
			if (orig) {
				origtext = `{{diff2|${orig.revid}|${orig.timestamp}}} ${hasHiddenComment(
					orig
				)}`;
			}
			const grouped_diffs = {};
			...
			const difftext = $.map(grouped_diffs, (sub) => {
				...
				if (sub.length >= 2) {
					...
					const label = `Consecutive edits made from ${new Morebits.date(
						first.timestamp
					).format('HH:mm, D MMMM YYYY', 'utc')} (UTC) to ${new Morebits.date(
						last.timestamp
					).format('HH:mm, D MMMM YYYY', 'utc')} (UTC)`;
			...
			if (params.free_resolves) {
				...
				if (page.compare) {
					...
					if (revCount < 3) {
						...
						resolvetext += `\n#  {{diff2|${rev.revid}|${new Morebits.date(
							rev.timestamp
						).format('HH:mm, D MMMM YYYY', 'utc')} (UTC) on ${
							page.title
						...
						const label = `Consecutive edits made from ${new Morebits.date(
							...
							revLatest.timestamp
						).format('HH:mm, D MMMM YYYY', 'utc')} (UTC) on ${page.title}`;
