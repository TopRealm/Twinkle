...
Twinkle.batchprotect.callback = () => {
	...
	form.append({
		...
		event: (e) => {
			...
			e.target.form.createexpiry.disabled =
					!e.target.checked || e.target.form.createlevel.value === 'all';
			e.target.form.createlevel.style.color = e.target.form.createexpiry.style.color = e
				.target.checked
				? ''
				: 'transparent';
		},
	...
	if (mw.config.get('wgNamespaceNumber') === 14) {
		...
		query.gapnamespace =
				mw.util.getParamValue('namespace') || $('select[name=namespace]').val();
...
Twinkle.batchprotect.callback.evaluate = (event) => {
	...
	if (
		numProtected > 0 &&
			!confirm(
				`您即将对${mw.language.convertNumber(numProtected)}个全保护页面进行操作。您确定吗？`
			)
	) {
		return;
	}
...
Twinkle.batchprotect.callbacks = {
	main: (apiobj) => {
		...
		page.protect(
			apiobj.params.batchOperation.workerSuccess,
			apiobj.params.batchOperation.workerFailure
		);
	}
};
