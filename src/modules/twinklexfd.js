...
Twinkle.xfd.callbacks = {
	afd: {
		main: (pageobj) => {
			...
			if (params.usertalk) {
				...
				if (initialContrib === mw.config.get('wgUserName')) {
					pageobj
						.getStatusElement()
						.warn(`您（${initialContrib}）创建了该页，跳过通知`);
					initialContrib = null;
				} else {
		...
		taggingArticle: (pageobj) => {
			...
			pageobj.setEditSummary(
				`页面存废讨论：[[${params.logpage}#${Morebits.pageNameNorm}]]`
			);
	...
	ffd: {
		...
		taggingImage: (pageobj) => {
			...
			const params = pageobj.getCallbackParameters();
			pageobj.setPageText(
				`{{ifd|${Morebits.string.formatReasonText(
					params.reason
				)}|date={{subst:#time:c}}}}\n${text}`
			);
			pageobj.setEditSummary(
				`文件存废讨论：[[${params.logpage}#${Morebits.pageNameNorm}]]`
			);
			pageobj.setChangeTags(Twinkle.changeTags);
			pageobj.setWatchlist(Twinkle.getPref('xfdWatchPage'));
...
Twinkle.xfd.callback.evaluate = (e) => {
	...
	switch (type) {
		...
		case 'ffd': {
			...
			// Tagging file
			qiuwen_page = new Morebits.wiki.page(
				mw.config.get('wgPageName'),
				'加入存废讨论模板到文件描述页'
			);
			qiuwen_page.setFollowRedirect(false);
