...
Morebits.wiki.page = function (pageName, status) {
	 ...
	 * @returns {boolean}
	 */
	const fnCanUseMwUserToken = function (action) {
	 ...
	 */
	const fnNeedTokenInfoQuery = function (action) {
		const query = {
			action: 'query',
			meta: 'tokens',
			type: 'csrf',
	...
	const fnAutoSave = function (pageobj) {
	 ...
	 * @returns {boolean}
	 */
	const fnApplyWatchlistExpiry = function () {
	...
	const fnSaveError = function () {
		...
		if (errorCode === 'editconflict' && ctx.conflictRetries++ < ctx.maxConflictRetries) {
			...
			const purgeApi = new Morebits.wiki.api('检测到编辑冲突，正在更新服务器缓存', purgeQuery, function () {
	...
	const fnProcessChecks = function (action, onFailure, response) {
		...
		const actionMissing = missing && [ 'delete', 'move' ].indexOf(action) !== -1;
...
Morebits.wiki.preview = function (previewbox) {
	...
	this.beginRender = function (wikitext, pageTitle, sectionTitle) {
		...
		const renderApi = new Morebits.wiki.api('加载中…', query, fnRenderSuccess, new Morebits.status('预览'));
		renderApi.post();
	};
	const fnRenderSuccess = function (apiobj) {
		const response = apiobj.getResponse();
...
Morebits.userspaceLogger = function (logPageName) {
	...
	this.log = function (logText, summaryText) {
		...
		const page = new Morebits.wiki.page('User:' + mw.config.get('wgUserName') + '/' + logPageName, '将项目加入到用户空间日志'); // make this '... to ' + logPageName ?
